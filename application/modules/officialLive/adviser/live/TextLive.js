...
module.exports = React.createClass({
    ...
    setRoom: function () {
        Event.trigger('OpenDialog', {
            module: <SetLive onChange={this.setLiveData} info={1}/>,
            title: '直播室设置',
            width: 662,
            height: 568
        });
    ...
    getAdviser: function () {
        ...
        if (LiveHandle.isRoomOwner() && Config.CACHE_DATA.ROOM.status != 1)
            return <div className="live_operate">
                {/* <input type="button" value="开始直播" onClick={this.startRoom} ref="switchButton" /> */}
        ...
        return <div className="graphic_box">
            <div className="live_box">
                    ...
                    <ul className="right">
                        {
                            this.props.history ?
                                null :
                                <li style={{border:'none'}}>参与人数：<label>{Config.CACHE_DATA.ROOM.visitor_count}</label> 人</li>
                        }
                ...
                <div className={!this.props.history ? 'chat_content' : 'chat_content full_box'}>
                    ...
                    <div style={{bottom:'60%',width:'400px',marginLeft:'-200px',left:'50%'}} className={this.state.error ? 'shadow show' : 'shadow'} ref="shadow">
                        {this.state.error}
                    </div>
                </div>
            </div>
