module.exports = {
    login: function (callback, force) {
        if ((window.webkit && window.webkit.messageHandlers) || (window.android && window.android.pushEvent)) {
            ...
            try{
                if (force == true) {
                    result = JSON.parse(window.getAccessToken(force));
                } else {
            ...
            Forms.post({
                ...
                params: {code: Config.REFRESH_TOKEN},
                callback: function(result) {
                    if (result.code == 200) {
                        Config.ACCESS_TOKEN = result.data.access_token;
                        window.ACCESS_TOKEN = result.data.access_token;
                    }
                    callback(result);
                }
            });
        ...
        Forms.post({
            ...
            params: {
                ...
                // name: '18611311804',
                // password: Utils.md5('111111', 16),

                name: sessionStorage.getItem("NAME"),
                // name: '13521929879',
                password: Utils.md5(sessionStorage.getItem("PASSWORD"), 16),
            ...
            callback: function(result) {
                if (result.code == 200) {
                    Config.ACCESS_TOKEN = result.data.access_token;
                    window.ACCESS_TOKEN = result.data.access_token;
                    localStorage.setItem('token', Config.ACCESS_TOKEN);
                }
                callback(result);
            }
        });
    ...
    jsCallNative: function(str, data) {
        var postData = JSON.stringify({nativeCallJS: str, messageBody: data});
    ...
    pushMessage: function(event, data) {
        ...
        console.log("window.pushMessage",window.pushMessage);
    ...
    popNotify: function(icon, title, content, data, id) {
        ...
        if (typeof(window.popNotify) == 'function') {
            console.log(icon, title, content, data, id);
